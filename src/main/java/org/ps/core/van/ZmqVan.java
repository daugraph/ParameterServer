...
public class ZmqVan extends Van {
    ...
    public int bind(Node node, int maxRetry) {
        ...
        for (int i = 0; i < maxRetry + 1; ++i) {
            ...
            if (receiver.bind(address)) break;
            if (i == maxRetry) {
                port = -1;
            } else {
    ...
    public void connect(Node node) {
        ...
        if ((node.getRole() == getMyNode().getRole())
                && (!node.getId().equals(getMyNode().getId()))) {
            return;
        }
        ...
        if (getMyNode().getId() != null) {
            ...
            String watermark = Environment.getInstance().find("DMLC_PS_WATER_MARK");
            if (watermark != null) {
